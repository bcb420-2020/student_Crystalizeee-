...
library(tidyverse)
BiocManager::install(setdiff("GEOquery", rownames(installed.packages())), update = FALSE, version = "3.10")
library(GEOquery)       # Bioconductor

packages = c("BiocManager", "knitr", "rlang", 
             "devtools", "reshape2", "kableExtra","circlize", "RCurl",
             "png", "grid", "htmltools", "ggplot2", "cowplot",
             "magick", "readxl", "GSA", "VennDiagram")

bioconductor_packages = c("edgeR", "biomaRt", "gprofiler2", "ComplexHeatmap", "HGNChelper")
BiocManager::install(setdiff(bioconductor_packages, rownames(installed.packages())), update = FALSE, version = "3.10")
...
save.ranked.list.return.file.name <- function(fit, coeff, prefix = ""){
  ...
  setwd("~")
  directory <- getwd()
  write.table(ranked.list, file = file.path(directory, file.name), sep = "\t", row.names = F, quote = F)
  return(file.name)
}
...
**Heatmap of Patient B vs Patient C Top Hits**
...
---
nocite: |
  @kucera2016autoannotate
  @morris2011clustermaker
  @oesper2011wordcloud
  @ritchie2015limma
  @raudvere2019g
  @gu2016complex
